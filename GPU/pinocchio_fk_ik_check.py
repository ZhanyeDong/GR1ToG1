...
def estimate_base_alignment(
    ...
    t = t + np.array([0.13, 0.0, 0.])
    return R, t


class ExampleChecker:
    def __init__(self):
        self.urdf_g1 = "urdf/g1_29dof.urdf"
        self.urdf_gr1 = "urdf/GR1T2.urdf"
        ...
        self.g1_base = "pelvis"
        self.gr1_base = "base"
        self.g1_right_ee = "right_rubber_hand"
        self.gr1_right_ee = "r_hand_pitch"

        self.model_g1, self.data_g1 = load_model(self.urdf_g1)
        self.model_gr1, self.data_gr1 = load_model(self.urdf_gr1, strip_collision=True)

        self.R_align, self.t_align = estimate_base_alignment(
            self.model_g1,
            self.data_g1,
            self.g1_base,
        ...
        self.q0_full_g1 = pin.neutral(self.model_g1)

    def check(self, q_right_gr1):
        if isinstance(q_right_gr1, torch.Tensor):
            q_right_gr1 = q_right_gr1.detach().cpu().numpy()
        q_full_gr1 = set_right_arm_q(self.model_gr1, self.gr1_right_arm, q_right_gr1)
        gr1_pos, _ = compute_ee_pose(self.model_gr1, self.data_gr1, q_full_gr1, self.gr1_right_ee)
        g1_target = transform_gr1_pos_to_g1(gr1_pos, self.R_align, self.t_align)
        ...
        ik_result = ik_right_arm_position(
            ...
            self.g1_right_ee,
            g1_target,
            q0_full=self.q0_full_g1,
        )
        return ik_result.success


_EXAMPLE_CHECKER = None


def get_example_checker() -> ExampleChecker:
    global _EXAMPLE_CHECKER
    if _EXAMPLE_CHECKER is None:
        _EXAMPLE_CHECKER = ExampleChecker()
    return _EXAMPLE_CHECKER


def example(q_right_gr1):
    checker = get_example_checker()
    return checker.check(q_right_gr1)
...
if __name__ == "__main__":
    ...
    tensor = torch.load("action_list0318_1.pt")
    t=0
