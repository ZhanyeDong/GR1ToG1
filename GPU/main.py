...
def plane_angle_loss(j1, j3, j4, ground_normal=None, eps=1e-6):
    # Plane 1: defined by (j1, j3, j4)
    n1 = torch.cross(j3 - j1, j4 - j1, dim=-1)
    # Plane 2: contains (j1, j4) and is vertical to ground (uses ground_normal)
    if ground_normal is None:
        ground_normal = torch.tensor([0.0, 0.0, 1.0], device=j1.device, dtype=j1.dtype)
    if ground_normal.dim() == 1:
        ground_normal = ground_normal.unsqueeze(0).expand_as(j1)
    n2 = torch.cross(j4 - j1, ground_normal, dim=-1)

    n1_n = n1 / (torch.norm(n1, dim=-1, keepdim=True) + eps)
    n2_n = n2 / (torch.norm(n2, dim=-1, keepdim=True) + eps)
    cos_angle = torch.sum(n1_n * n2_n, dim=-1)
    return 1.0 - cos_angle


def _broadcast_point(p, ref):
    # Ensure point p has shape (B, 3) to match ref which is (B, 3).
    if p.dim() == 1:
        return p.unsqueeze(0).expand_as(ref)
    return p
...
def calculate_IK_loss(
    ...
    GR1_joint1 = _broadcast_point(GR1_joint1, GR1_joint3)
    G1_joint1 = _broadcast_point(G1_joint1, G1_joint3)
    ...
    plane_loss_gr1 = plane_angle_loss(GR1_joint1, GR1_joint3, GR1_joint4)
    plane_loss_g1 = plane_angle_loss(G1_joint1, G1_joint3, G1_joint4)
    plane_loss = F.mse_loss(plane_loss_gr1, plane_loss_g1)
...
def main(args):
    ...
    train_num = []
    test_num = []
    loaded_tensor = torch.load('train_dataset_0322.pt')

    for i in range(len(loaded_tensor)):
        joint_angles = torch.tensor(loaded_tensor[i])
    ...
    data_loader_train = DataLoader(data, batch_size=args.batch_size, shuffle=True, pin_memory=pin_memory)
    epochs = args.epochs
    model = MLP_self(7, 32, 128, 128, 32, 7).to(device)
    ...
    torch.save(model, 'model_complete0322_2.pth')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size', type=int, default=4096, help='input batch size for training (default: 1)')
    parser.add_argument('--learning_rate', type=float, default=0.001, help='learning rate (default: 0.003)')
    parser.add_argument('--epochs', type=int, default=15000, help='gradient clip value (default: 300)')
