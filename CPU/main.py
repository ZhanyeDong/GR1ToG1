...
def main(args):
    ...
    model = MLP_self(num_i, num_h1,num_h2,num_h3,num_h4,num_o)
    ...
    output_list = []
    sevenD_joint3_list = []
    GR1_DOF = FK_GR1_7DOF.FK_7DOF()
    G1_DOF = FK_G1_7DOF.FK_7DOF()
    for epoch in range(epochs):
        ...
        for (inputs,) in data_loader_train:  # 读入数据开始训练
            ...
            # print(end_eff_calcu_by_FK)
                ...
                if epoch == epochs - 1:
                    sevenD_angle = intermediate_outputs[i]
                    data_list.append(inputs[i])
                    sevenD_angle_list.append(sevenD_angle)
                    output_list.append(intermediate_outputs[i])
                    sevenD_joint3_list.append(G1_joint3)
                GR1_joint1 = torch.tensor([0.19346,0. , 0.0])
                G1_joint1 = torch.tensor([0.15596,-0.0025,-0.05])
...
#测试集
    ...
    for data_test in data_loader_test:  # 读入数据开始训练
        ...
        for i in range(len(inputs_test)):
            ...
            data_test_list.append(inputs_test[i])
            output_test_list.append(intermediate_outputs_test[i])
            GR1_joint1 = torch.tensor([0.19346, 0., 0.])
            G1_joint1 = torch.tensor([0.15596, -0.0025, -0.05])
            tamp = calculate_IK_loss(GR1_joint1, G1_joint1, GR1_joint3, G1_joint3, GR1_joint4, G1_joint4
                                     , GR1_position, GR1_orientation, G1_position, G1_orientation)
            IK_loss_batch_test = IK_loss_batch_test + tamp
        loss_test = (IK_loss_batch_test) / len(inputs_test)
        sum_test_loss = sum_test_loss + loss_test.data
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size', type=int, default=4, help='input batch size for training (default: 1)')
